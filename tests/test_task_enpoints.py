...
class TestUpdateTask:
    async def test_update_task_positive_case(
        self, async_client: AsyncClient, auth_headers: dict, task: dict
    ):
        new_task_data = {
            "title": "new task1",
        ...
        response = await async_client.patch(
            f"/tasks/{task_id}", json=new_task_data, headers=auth_headers
        )
    ...
    async def test_update_task_negative_cases(
        ...
        response = await async_client.patch(
            f"/tasks/{task_id + 1}", json=new_task_data, headers=auth_headers
        )
...
class TestDeleteTask:
    ...
    async def test_delete_task_negative_cases(
        ...
        response = await async_client.delete(
            f"/tasks/{task_id + 1}", headers=auth_headers
        )
        ...
        response = await async_client.delete(
            f"/tasks/{task_id}", headers=second_headers
        )
...
class TestGetTask:
    async def test_get_tasks_positive(
        ...
        response = await async_client.post(
            "/tasks/", json=new_task_data, headers=auth_headers
        )
        ...
        expected_tasks = [
            {"id": task_id_1, **task_data, **default_data},
            {"id": task_id_2, **new_task_data, **default_data},
        ]
    ...
    async def test_get_tasks_negative_cases(
        ...
        response = await async_client.get(f"/tasks/{task_id + 1}", headers=auth_headers)
